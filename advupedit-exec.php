...
$advclosedate=$_POST['closedate'];
$advdepartment=$_POST['department'];
$advreference=$_POST['advreference'];
$advID=$_POST['advID'];
...
// if save button on the form is clicked
    ...
    if (!in_array($extension, ['pdf', 'docx', 'doc'])) {
        ...
        if (move_uploaded_file($file, $destination)) {
			...
			$sql = "UPDATE advup SET advtitle='$advtitle',advclosedate='$advclosedate', advfile='$filename', advdept='$advdepartment', advreference = '$advreference' WHERE advID='$advID'";
	...
	$sql = "UPDATE advup SET advtitle='$advtitle',advclosedate='$advclosedate', advdept='$advdepartment', advreference = '$advreference' WHERE advID='$advID'";
		if ($conn->query($sql) === TRUE) {
			$anotherfeed =  '<div class="alert alert-success alert-dismissible fade show">
			<strong>Success!</strong> Your data has been inserted successfully.
			<button type="button" class="close" data-dismiss="alert">&times;</button>
			</div>';
		} else {
		$anotherfeed1 =  '<div class="alert alert-danger alert-dismissible fade show">
        <strong>Error!</strong> A problem has been occurred while submitting your data.
        <button type="button" class="close" data-dismiss="alert">&times;</button>
    </div>';
}
